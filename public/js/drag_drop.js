...
/***/ "./resources/assets/js/custom/custom_drag_drop.js":
/*!********************************************************!*\
  !*** ./resources/assets/js/custom/custom_drag_drop.js ***!
  \********************************************************/
...
if ($(".drag-area").length > 0) {
  var showFile = function showFile() {
    var fileType = file.type;
    var size = file.size;

    if (size > 2097152) {
      shownotify('File too Big, please select a file less than 2mb', 'danger');
      return false; //getting selected file type
    }

    var validExtensions = ["image/pdf", "image/jpg", "image/png", "image/jpeg", "image/PDF", "image/JPG", "image/PNG", "image/JPEG"]; //adding some valid image extensions in array
    ...
    if (validExtensions.includes(fileType)) {
      ...
      fileReader.onload = function () {
        ...
        var elem = 'false';
        var index = $('#image_type').val();
        $(".doc-verify").each(function (key) {
          var img_position = $(this).attr('img-position');

          if ($(this).attr('img-type') == 'dynamic' && img_position == index) {
            shownotify('You have already selected this image type. Click on close icon to select again', 'danger');
          } else if ($(this).attr('img-type') == 'custom' && img_position == index) {
            $('#img' + index + '').attr('src', fileURL);
            $('#img' + index + '').after('<img src="' + url + '/img/cancel.png" class="img-fluid close_img" data-index="' + index + '" type="">');
            $(this).attr('img-type', 'dynamic');
            var imgTag = "<img src=\"".concat(fileURL, "\" alt=\"\">");
            $('.custom-drop-area').attr('selected-img', index); // $('.custom-drop-area').html(imgTag);

            formData.append('file' + index, file);
            var next_index = parseInt(index) + 1;
            prev = parseInt(index) - 1; // if($('#img'+next_index+'').attr('img-type') != 'dynamic' && next_index < 4) {
            //     $('#image_type').val(next_index);   
            // }else if($('#img'+prev+'').attr('img-type') != 'dynamic' && prev > 0){
            //     $('#image_type').val(prev);
            // }
          }
        });
      ...
      shownotify('This is not an valid Image File! ', 'danger');
      return false;
    }
  };

  var shownotify = function shownotify(message, type) {
    $.notify({
      title: "",
      message: message
    }, {
  ...
  input.addEventListener("change", function () {
    ...
    file = this.files[0]; // console.log(formData.getAll('file'));
  ...
  dropArea.addEventListener("dragleave", function () {// dropArea.classList.remove("active");
  ...
  $('body').on('click', '.close_img', function () {
    ...
    $('#img' + index).attr('src', url + '/img/icon-id-card.png');
    $('#img' + index).attr('img-type', 'custom');

    if ($('.custom-drop-area').attr('selected-img') == index) {
      $('.custom-drop-area').html('');
    }
  ...
  $('body').on('click', '.upload-file', function () {
    var file_count_verified = true;

    if (formData.getAll('file1').length == 0 && formData.getAll('file2').length == 0 && formData.getAll('file3').length == 0) {
      file_count_verified = false;
    }

    if (file_count_verified == true) {
      $.ajaxSetup({
        headers: {
      ...
      $.ajax({
        ...
        success: function success(response) {
          if (response.status == true) {
            shownotify(response.message, 'success'); // formData.delete('file1');
            // formData.delete('file2');
            // formData.delete('file3');
            // $(".verification-content-block .close_img").trigger('click');
            // $('#image_type').val('1');
            // $("input[name='resend']").val('0');
            // $(".verification-messages").html('<p class="info"><span class="danger">Note:</span> Admin will review your documents. Once your document will be approved, you can access the panel.</p>');
            // $("#verify_doc_block").addClass('d-none');
        ...
        error: function error(response) {
          shownotify('Failed to upload images', 'danger');
        }
      });
    } else {
      shownotify('Please select atleast one document.', 'danger');
    }
  });
}
...
/******/ 	__webpack_require__("./resources/assets/js/bootstrap-notify.min.js");
/******/ 	var __webpack_exports__ = __webpack_require__("./resources/assets/js/custom/custom_drag_drop.js");
/******/ 	
/******/ })()
;